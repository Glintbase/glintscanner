...
export default function ResultsReport({ score, checks, scanId }: { score: number; checks: any[]; scanId?: string }) {
  ...
  const shareUrl = scanId ? `https://glintbase.xyz/scan/${scanId}` : 'https://glintbase.xyz/scan';
  const tweetText = `Just scanned my docs with the @glintbase Agent Readiness Scanner.\n\nScore: ${score}/100 — ${label}\n\nSee how AI agents rate your documentation:`;
      ...
      >
        ...
        <div className="flex-1 text-center md:text-left space-y-3">
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center gap-2 mt-2 text-[11px] font-black uppercase tracking-widest text-white/40 hover:text-white border border-white/10 hover:border-white/30 rounded-lg px-4 py-2 transition-all"
          >
